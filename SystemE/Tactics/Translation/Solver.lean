import Lean
import Smt.Solver
import SystemE.Tactics.Translation.EuclidTheory
import SystemE.Tactics.Translation.Translator

open Lean Elab Tactic Meta Smt Solver
...
def getTraceOption : MetaM Bool := do
  match systemE.trace.get? (← getOptions) with
...
private def fromEAssertion (e : EAssertion) : Smt.Command :=
  e |> EAssertionToString |> Smt.Term.literalT |> Smt.Command.assert

/- Translate the entire ESMT context to a list of SMT commands -/
private def fromEsmt : Esmt → List Smt.Command
| ⟨consts, _, asserts, _⟩ =>
